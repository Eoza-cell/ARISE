...
```    getImagePath(name) {
        return path.join(this.tempPath, `${name}_${Date.now()}.png`);
    }

    async generateFallbackImage(type = 'fallback') {
        console.log(`🎨 Génération image de fallback pour: ${type}`);
        const imagePath = this.getImagePath(`fallback_${type}`);

        if (this.hasFreepik && this.freepikClient) {
            try {
                await this.freepikClient.generateFallbackImage(imagePath, type);
                const imageBuffer = await fs.readFile(imagePath).catch(() => null);
                if (imageBuffer) {
                    console.log(`✅ Image fallback générée par Freepik`);
                    return imageBuffer;
                }
            } catch (freepikError) {
                console.log('⚠️ Erreur Freepik fallback:', freepikError.message);
            }
        }

        // Si tout échoue, créer une image vierge basique
        try {
            await sharp({
                create: {
                    width: 1024,
                    height: 576,
                    channels: 4,
                    background: { r: 30, g: 30, b: 30, alpha: 1 } // Fond sombre
                }
            })
                .png()
                .toFile(imagePath);
            ...
            const imageBuffer = await fs.readFile(imagePath).catch(() => null);
            if (imageBuffer) {
                console.log('✅ Image fallback basique générée');
                return imageBuffer;
            }
        } catch (sharpError) {
            console.error('❌ Erreur génération image fallback basique:', sharpError);
        }

        throw new Error(`Impossible de générer une image de fallback pour ${type}`);
    }

    async generateCombatScene(character1, character2, combatLog, options = {}) {
        try {
            const cacheKey = `combat_scene_${character1.id}_${character2.id}`;
            if (this.imageCache.has(cacheKey)) {
                return this.imageCache.get(cacheKey);
            }

            const imagePath = this.getImagePath(`combat_scene_${character1.id}_vs_${character2.id}`);

            // Utiliser d'abord Pollinations pour une scène de combat dynamique
            if (this.hasPollinations && this.pollinationsClient) {
                try {
                    console.log(`🎨 Génération scène de combat avec Pollinations GRATUIT...`);
                    const prompt = `Epic fantasy battle scene between ${character1.name} and ${character2.name}, ${character1.name} is attacking ${character2.name}, dynamic action, cinematic lighting, fantasy art style, high detail, first person POV for ${character1.name}`;
                    await this.pollinationsClient.generateCombatScene(prompt, imagePath, { style: options.style || this.defaultStyle, perspective: 'first_person', nudity: this.allowNudity });
                    const imageBuffer = await fs.readFile(imagePath).catch(() => null);
                    if (imageBuffer) {
                        console.log(`✅ Scène de combat générée par Pollinations GRATUIT`);
                        this.imageCache.set(cacheKey, imageBuffer);
                        return imageBuffer;
                    }
                } catch (pollinationsError) {
                    console.log('⚠️ Erreur Pollinations combat, fallback vers Freepik:', pollinatorsError.message);
                }
            }
            ...
            if (this.hasFreepik && this.freepikClient) {
                try {
                    console.log(`🎨 Génération scène de combat avec Freepik...`);
                    const prompt = `Fantasy battle between ${character1.name} and ${character2.name}. ${character1.name} (${character1.gender}) is fighting ${character2.name} (${character2.gender}). ${character1.name} is from ${character1.kingdom} and ${character2.name} is from ${character2.kingdom}. Action shot, dynamic poses, dramatic lighting, fantasy art.`;
                    await this.freepikClient.generateImage(prompt, imagePath, { style: options.style || this.defaultStyle, perspective: 'third_person', nudity: this.allowNudity });
                    const imageBuffer = await fs.readFile(imagePath).catch(() => null);
                    if (imageBuffer) {
                        console.log(`✅ Scène de combat générée par Freepik`);
                        this.imageCache.set(cacheKey, imageBuffer);
                        return imageBuffer;
                    }
                } catch (freepikError) {
                    console.log('⚠️ Erreur Freepik combat, fallback vers KieAI:', freepikError.message);
                }
            }
            ...
            if (this.hasKieAI && this.kieaiClient) {
                try {
                    console.log(`🎨 Génération scène de combat avec KieAI...`);
                    await this.kieaiClient.generateCombatScene(character1, character2, combatLog, imagePath, { style: options.style || this.defaultStyle, perspective: 'third_person', nudity: this.allowNudity });
                    const imageBuffer = await fs.readFile(imagePath).catch(() => null);
                    if (imageBuffer) {
                        console.log(`✅ Scène de combat générée par KieAI`);
                        this.imageCache.set(cacheKey, imageBuffer);
                        return imageBuffer;
                    }
                } catch (kieaiError) {
                    console.log('⚠️ Erreur KieAI combat:', kieaiError.message);
                }
            }

            return await this.generateFallbackImage('Combat Scene');

        } catch (error) {
            console.error('❌ Erreur génération scène de combat:', error);
            return await this.generateFallbackImage('Combat Scene Error');
        }
    }

    async generateMapImage(mapType, mapData, options = {}) {
        try {
            const cacheKey = `map_${mapType}_${options.style || this.defaultStyle}`;
            if (this.imageCache.has(cacheKey)) {
                return this.imageCache.get(cacheKey);
            }

            const imagePath = this.getImagePath(`map_${mapType}`);

            // Utiliser notre générateur personnalisé pour les cartes du monde (style Inkarnate)
            if (mapType === 'world_map') {
                console.log('🗺️ Génération carte du monde HAUTE QUALITÉ...');
                const worldMapGenerator = new WorldMapGenerator();
                const mapBuffer = await worldMapGenerator.generateWorldMap(imagePath);
                console.log('✅ Carte du monde générée avec succès');
                this.imageCache.set(cacheKey, mapBuffer);
                return mapBuffer;
            }

            // Fallback pour d'autres types de cartes (région, ville, etc.)
            console.log(`🎨 Génération carte (${mapType}) avec FreepikClient...`);
            let prompt = `Fantasy ${mapType} map, detailed, cartography,${options.style ? ` ${options.style} style,` : ''} intricate details, clear labels for regions and landmarks.`;

            if (mapData && mapData.description) {
                prompt += ` ${mapData.description}`;
            }

            await this.freepikClient.generateImage(prompt, imagePath, {
                style: options.style || this.defaultStyle,
                perspective: 'top_down', // Carte vue de dessus
                nudity: false
            });

            const imageBuffer = await fs.readFile(imagePath).catch(() => null);
            if (imageBuffer) {
                console.log(`✅ Carte (${mapType}) générée par Freepik`);
                this.imageCache.set(cacheKey, imageBuffer);
                return imageBuffer;
            }

            return await this.generateFallbackImage(`Map ${mapType}`);

        } catch (error) {
            console.error(`❌ Erreur génération carte ${mapType}:`, error);
            return await this.generateFallbackImage(`Map ${mapType} Error`);
        }
    }

    // Méthode pour obtenir le chemin d'une image dans le dossier temporaire
    getImagePath(name) {
        return path.join(this.tempPath, `${name}_${Date.now()}.png`);
    }
}
